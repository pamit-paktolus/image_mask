...
def mask_maker_v2(IMAGE_PATH):
    ...
    try:
        ...
        return "Some issue with loading the image , please check image path"
    if image is None:
        return "Some issue with loading the image , please check image path"
    ...
    if len(result) == 0 or result[0] is None:
        ...
        return "document not in the list of supported documents or picture quality is not up to mark"
    ...
    for detection in result[0]:
        ...
        if is_unsupported_doc(text):
            valid_doc = False
            cv2.imwrite('unsupported/' + img_name + '_uns.'+ ext, image_org)
            return "document not in the list of supported"
    ...
    if clear_img:
        try:
            cv2.imwrite(masked_image_dir + img_name + '_m.'+ext, image)
            ...
            return "Not able to write image on disk, please check for writing rights"
        
    elif valid_doc:
        try:
            cv2.imwrite('not_clear/' + img_name + '_nc.'+ ext, image_org)
            return "picture quality is not up to mark"
        except:
            return "Not able to write image on disk, please check for writing rights"
    try:
        cv2.imwrite('unsupported/' + img_name + '_uns.'+ ext, image_org)
        return "document not in the list of supported documents"
    except:
        return "Not able to write image on disk, please check for writing rights"
